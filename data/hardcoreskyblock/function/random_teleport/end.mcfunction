...
execute at @s as @a[scores={hcsb.rtp_id=0}] run tp ~0.5 ~10 ~2.5
execute as @a[scores={hcsb.rtp_id=0}] run rotate @s 180 0
tag @s remove hcsb.rtp_end
tag @a[scores={hcsb.rtp_id=0}] add hcsb.end_rtp
schedule function hardcoreskyblock:random_teleport/tp_player 5t
